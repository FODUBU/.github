FODUBU® — Force de Développement et Ubuntu au Burundi

🌍 About FODUBU

FODUBU® is a dynamic, multi-sector digital platform based in Burundi, built to empower businesses, communities, and consumers through transparent, inclusive, and globally connected commerce.

FODUBU operates a unified ecosystem of three decentralized applications (DApps) within the Pi Network ecosystem, combining e-commerce, blockchain-based payments, and community-driven governance.

FODUBU Ecosystem DApps

## 🎯 The Three Applications

### 1. FODUBU - Main Unified App
- **URL**: https://trade.fodubu.com
- **Purpose**: Central marketplace and utility hub
- **Role**: Trade and service aggregation, entry point to ecosystem

### 2. TRACO - Agri-Connect Marketplace
- **URL**: https://traco.fodubu.com
- **Purpose**: Digital agricultural marketplace
- **Features**:
  - Buying/selling agricultural products
  - Seed distribution to branches and farmers
  - Agricultural value-chain connectivity

### 3. SETRA - Eco Marketplace
- **URL**: https://setra.fodubu.com (also https://setra2493.pinet.com)
- **Purpose**: Environment-focused sustainable trade
- **Features**:
  - Reforestation, Aquaculture, Transportation, Clean Energy, Construction


## 🏗️ Technical Architecture

### Current Stack 

#### Frontend
- ReactJS with Vite
- NextJS with Next
- Pi JavaScript SDK (`@pi-apps/pi-sdk`)
- Pi Browser-compatible UI

#### Backend (fodubu-unified-api)
- **URL**: https://api.fodubu.com
- Node.js + Express.js
- Pi Backend SDK (`@pi-apps/backend`)
- Stellar SDK (`@stellar/stellar-sdk`)
- Secure Pi payment verification

#### Database
- MongoDB
- Environment-based configuration
- CI-ready structure

-

## 🔗 Repository Structure

### Main Repositories
1. **PiOS (This repo)**: https://github.com/FODUBU/PiOS
   - Documentation and project overview
   - Open source component

2. **fodubu-api**: https://github.com/FODUBU/fodubu-api
   - Unified backend for all 3 apps
   - Private repository (read-only access on request)

3. **Frontend Repos**
   - FODUBU Main frontend: https://github.com/FODUBU/fodubu
   - TRACO-MARKETPACE  v0: https://github.com/FODUBU/TRACO-MARKETPLACE
   - SETRA-MARKETPLACE v0: https://github.com/FODUBU/SETRA-MARKETPLACE


FODUBU transforms traditional trade ecosystems into transparent, trusted, and globally accessible marketplaces.

🤝 Community & Contribution

We welcome everyone to learn, explore, and participate:

🔍 Search for FODUBU DApps in the Pi Browser

🛍 Learn while shopping and trading

🧠 Share ideas and feedback with the community

Community growth and collaboration are central to FODUBU’s mission.

📚 Useful Resources

🐦 X (Twitter): https://x.com/Fodubu?s=09

📺 YouTube: https://youtube.com/@fodubucommunity

📦 GitHub Organization: https://github.com/orgs/FODUBU


🔐 Long-Term Vision

FODUBU, S.E.TRA, and FODUBU Trade-Agri-Connect are built for:

Long-term sustainability

The Pi Network ecosystem

Community-driven economic growth


Our goal is lasting impact, not short-term speculation.

🍿 Fun Fact

The FODUBU team often starts the day with fried potatoes, eggs, and coffee or tea ☕🥔🍳

✍️ Open-Source Standards

The FODUBU team follows GitHub’s official writing and formatting guidelines:
https://docs.github.com/github/writing-on-github/getting-started-with-writing-and-formatting-on-github/basic-writing-and-formatting-syntax


© FODUBU® — Force de Développement et Ubuntu au Burundi

Building inclusive trade, powered by community and Pi Network.
